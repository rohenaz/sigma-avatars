...
const REGISTRY_URL =
  "https://raw.githubusercontent.com/jnsahaj/tweakcn/refs/heads/main/public/r/registry.json";
...
function extractColorsFromTheme(theme: RegistryTheme): string[] {
  ...
  if (colors.length < 5) {
    const chartKeys = Object.keys(lightVars).filter((key) =>
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (data.items && Array.isArray(data.items)) {
        ...
        themes = data.items.filter(
          (item: any) =>
            item && typeof item === "object" && item.type === "registry:style",
        ) as RegistryTheme[];
      } else {
        // If it's an object, extract values
        themes = Object.values(data).filter(
          (item) =>
            item &&
            typeof item === "object" &&
            (item as any).type === "registry:style",
        ) as RegistryTheme[];
      }
    }

    console.log(
      `Successfully fetched ${themes.length} themes from tweakcn registry`,
    );
