...
import { Shuffle, Circle, Square, SquareIcon, Loader2 } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export const Playground = () => {
  ...
  const {
    handleAvatarClick: contextHandleAvatarClick,
    selectedAvatar,
    setCurrentColors,
    setSelectedAvatar,
  } = useSidebarContext();
  ...
  const dotColors = useMemo(
    () => [dotColor0, dotColor1, dotColor2, dotColor3, dotColor4],
    [dotColor0, dotColor1, dotColor2, dotColor3, dotColor4],
  );
  const setters = [
    setDotColor0,
  ...
  useEffect(() => {
    if (data) {
      const total = data.pages.reduce(
        (acc, page) => acc + page.companies.length,
        0,
      );
  ...
  useEffect(() => {
    ...
    // Always check for api parameter, not just when name is present
    if (api === "true") {
      setUseApi(true);
    } else if (api === "false") {
      setUseApi(false);
    ...
    if (name) {
      const avatarData: SelectedAvatar = {
        ...
        (variants.includes(avatarVariant as Variant) ||
          avatarVariant === "random")
          ? avatarVariant
          : variant) as Variant,
        colors: dotColors,
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="sticky top-[var(--app-header-h,3.5rem)] z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          ...
          <div className="px-2 md:px-3 flex items-center gap-2 h-12">
            ...
            <Separator
              ...
              className="mx-2 hidden md:block h-6"
            />
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className="h-7 w-7"
                      onClick={() => {
                        const currentIndex = allPalettes.findIndex(
                          (p) => p.name === selectedTheme,
                        );
                        const prevIndex =
                          currentIndex > 0
                            ? currentIndex - 1
                  ...
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const nextIndex =
                          currentIndex < allPalettes.length - 1
                            ? currentIndex + 1
                            : 0;
        ...
        {/* Row 2: Size, Shape, Server toggle, and Colors */}
        <div className="px-2 md:px-3 flex items-center gap-2 h-10 border-t border-border/10">
          ...
          <div className="flex items-center gap-2">
            ...
            <Switch
              ...
              onCheckedChange={(checked) => {
                setUseApi(checked);
                // Update URL to persist server toggle state
                const params = new URLSearchParams(searchParams.toString());
                params.set("api", checked.toString());
                router.push(`?${params.toString()}`, { scroll: false });
              }}
            />
          </div>
          ...
          <ToggleGroup
            ...
            className="h-8"
          >
            <ToggleGroupItem value="circle" className="text-xs h-7 px-2">
              <Circle className="size-3.5" />
            </ToggleGroupItem>
            <ToggleGroupItem value="rounded" className="text-xs h-7 px-2">
              <Square className="size-3.5" />
            </ToggleGroupItem>
            <ToggleGroupItem value="square" className="text-xs h-7 px-2">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
          ...
          <div className="w-full min-h-[400px] grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
            {allCompanies.map((company, index) => {
              ...
              const displayVariant =
                variant === "random" ? variants[variantIndex] : variant;
              const paletteIndex =
                Math.floor(index / 50) % Math.max(allPalettes.length, 1);
              const avatarColors =
                variant === "random" && allPalettes.length > 0
