...
const EyeGroup: React.FC<{
  ...
  lidDrop: number; outline: string; pupil: string;
  ...
  return (
    <g>
      ...
      {/* pupil + specular */}
      <circle cx={cx + rx * 0.15} cy={cy - ry * 0.05} r={Math.max(2, rx * 0.42)} style={{ fill: pupil }} />
      <circle cx={cx + rx * 0.04} cy={cy - ry * 0.15} r={Math.max(1, rx * 0.15)} style={{ fill: '#fff' }} />
        ...
        <path
          d={`M ${cx - rx} ${cy - lidDrop}
              Q ${cx} ${cy - ry - lidDrop * 0.2}
                ${cx + rx} ${cy - lidDrop}`}
          style={{ stroke: outline, strokeWidth: 2, fill: 'none', strokeLinecap: 'round' }}
        />
      </g>
    </g>
  );
...
function Lips({
  x1, y, x2, curve, thickness, color, outline = '#000', split = '#2b1a12', tilt = 0,
}: { x1: number; y: number; x2: number; curve: number; thickness: number; color: string; outline?: string; split?: string; tilt?: number; }) {
  const cx = (x1 + x2) / 2;
  const cy = y + curve;
  ...
  return (
    <>
      <path d={d} style={{ stroke: outline, strokeWidth: thickness + 3, fill: 'none', strokeLinecap: 'round' }} {...tr} />
      <path d={d} style={{ stroke: color, strokeWidth: thickness, fill: 'none', strokeLinecap: 'round' }} {...tr} />
      <path d={d} style={{ stroke: split, strokeWidth: Math.max(1, thickness * 0.25), fill: 'none', strokeLinecap: 'round' }} {...tr} />
    </>
  );
}
...
export default function AvatarPepe({
  ...
  const sortedColors = [...fullColorPool].sort((a, b) => getBrightness(a) - getBrightness(b));
  
  // Background: always darkest available color
  let bg = sortedColors[0];
  ...
  if (skin === bg || Math.abs(getUnit(n + 143, 100)) < 10) {
    ...
    const contrastSkins = ['#6B9F2E', '#7FAF3F', '#FFB6C1', '#87CEEB', '#DDA0DD'];
    skin = contrastSkins[getUnit(n + 144, contrastSkins.length)];
  }
  
  if (lipColor === skin || lipColor === bg) {
    const contrastLips = ['#C03A2B', '#FF69B4', '#FF1493', '#DC143C', '#8B008B'];
  ...
  const lipCurve = isMad ? -baseCurve * 0.8 : (isCrying ? -baseCurve * 0.5 : baseCurve); // Sad when crying, frown when mad
  ...
  const eyeY = SIZE * (baseEyeY + (isFullFace ? eyeYVariation : 0)) - (isFullFace ? 0 : SIZE * 0.08);
  ...
  return (
    ...
    >
      ...
      {bgPattern >= 0 && (
        <defs>
          <pattern id={patternID} patternUnits="userSpaceOnUse" width={
            bgPattern === 0 ? SIZE * 0.2 : // vertical stripes
            bgPattern === 1 ? SIZE : // horizontal stripes  
            bgPattern === 2 ? SIZE * 0.25 : // polka dots
            bgPattern === 3 ? SIZE * 0.2 : // checkerboard
            SIZE * 0.2 // diagonal
          } height={
            bgPattern === 0 ? SIZE : // vertical stripes
            bgPattern === 1 ? SIZE * 0.2 : // horizontal stripes
            bgPattern === 2 ? SIZE * 0.25 : // polka dots
            bgPattern === 3 ? SIZE * 0.2 : // checkerboard
            SIZE * 0.2 // diagonal
          }>
            {bgPattern === 0 && (
              /* Vertical stripes */
              <>
                <rect width={SIZE * 0.1} height={SIZE} fill={String(bg)} />
                <rect x={SIZE * 0.1} width={SIZE * 0.1} height={SIZE} fill={String(skin)} opacity="0.2" />
              </>
            )}
            {bgPattern === 1 && (
              /* Horizontal stripes */
              <>
                <rect width={SIZE} height={SIZE * 0.1} fill={String(bg)} />
                <rect y={SIZE * 0.1} width={SIZE} height={SIZE * 0.1} fill={String(skin)} opacity="0.2" />
              </>
            )}
            {bgPattern === 2 && (
              /* Polka dots */
              <>
                <rect width={SIZE * 0.25} height={SIZE * 0.25} fill={String(bg)} />
                <circle cx={SIZE * 0.125} cy={SIZE * 0.125} r={SIZE * 0.05} fill={String(skin)} opacity="0.25" />
              </>
            )}
            {bgPattern === 3 && (
              /* Checkerboard */
              <>
                <rect width={SIZE * 0.1} height={SIZE * 0.1} fill={String(bg)} />
                <rect x={SIZE * 0.1} width={SIZE * 0.1} height={SIZE * 0.1} fill={String(skin)} opacity="0.2" />
                <rect y={SIZE * 0.1} width={SIZE * 0.1} height={SIZE * 0.1} fill={String(skin)} opacity="0.2" />
                <rect x={SIZE * 0.1} y={SIZE * 0.1} width={SIZE * 0.1} height={SIZE * 0.1} fill={String(bg)} />
              </>
            )}
            {bgPattern === 4 && (
              /* Diagonal stripes */
              <>
                <polygon points={`0,0 ${SIZE * 0.1},0 0,${SIZE * 0.1}`} fill={String(skin)} opacity="0.2" />
                <polygon points={`${SIZE * 0.1},0 ${SIZE * 0.2},0 ${SIZE * 0.2},${SIZE * 0.1} ${SIZE * 0.1},${SIZE * 0.2} 0,${SIZE * 0.2} 0,${SIZE * 0.1}`} fill={String(bg)} />
                <polygon points={`${SIZE * 0.2},${SIZE * 0.1} ${SIZE * 0.2},${SIZE * 0.2} ${SIZE * 0.1},${SIZE * 0.2}`} fill={String(skin)} opacity="0.2" />
              </>
            )}
          </pattern>
        </defs>
      ...
      <g mask={`url(#${maskID})`}>
        ...
        {isFullFace && (
          <rect width={SIZE} height={SIZE} style={{ fill: String(skin), opacity: 0.95 }} />
        )}

        {/* head - only draw if not full-face */}
        {!isFullFace && (
          <g transform={`rotate(${tilt} ${cx} ${cy})`}>
            <path d={frogBlobPath(cx, cy, n)} style={{ fill: String(skin), stroke: String(outline), strokeWidth: 2 }} />
          </g>
        )}

        {/* eyes (grouped + clipped lids; draw non-dominant first) */}
        {rightOnTop
          ? <EyeGroup cx={cx - sep / 2} cy={eyeY} rx={eyeRx} ry={eyeRy} lidDrop={lidDrop} outline={String(outline)} pupil={pupil} />
          : <EyeGroup cx={cx + sep / 2} cy={eyeY} rx={eyeRx} ry={eyeRy} lidDrop={lidDrop} outline={String(outline)} pupil={pupil} />
        }
        {rightOnTop
          ? <EyeGroup cx={cx + sep / 2} cy={eyeY} rx={eyeRx} ry={eyeRy} lidDrop={lidDrop} outline={String(outline)} pupil={pupil} />
        ...
        {isCrying && (
          <>
            ...
            <ellipse cx={cx - sep / 2} cy={eyeY + eyeRy * 0.5} rx={eyeRx * 0.9} ry={eyeRy * 0.4}
                     style={{ fill: '#4A90E2', opacity: 0.4 }} />
            <ellipse cx={cx + sep / 2} cy={eyeY + eyeRy * 0.5} rx={eyeRx * 0.9} ry={eyeRy * 0.4}
                     style={{ fill: '#4A90E2', opacity: 0.4 }} />
            
            {/* Tear drops */}
            <ellipse cx={cx - sep / 2 - eyeRx * 0.7} cy={eyeY + eyeRy * 1.5} rx={3} ry={5}
                     style={{ fill: '#4A90E2', opacity: 0.7 }} />
            <ellipse cx={cx + sep / 2 + eyeRx * 0.7} cy={eyeY + eyeRy * 1.5} rx={3} ry={5}
                     style={{ fill: '#4A90E2', opacity: 0.7 }} />
            
            {/* Tear streaks */}
            <path d={`M ${cx - sep / 2 - eyeRx * 0.7} ${eyeY + eyeRy} 
                      Q ${cx - sep / 2 - eyeRx * 0.7} ${eyeY + eyeRy * 2} 
                        ${cx - sep / 2 - eyeRx * 0.8} ${eyeY + eyeRy * 2.5}`}
                  style={{ stroke: '#4A90E2', strokeWidth: 2, fill: 'none', opacity: 0.3, strokeLinecap: 'round' }} />
            <path d={`M ${cx + sep / 2 + eyeRx * 0.7} ${eyeY + eyeRy} 
                      Q ${cx + sep / 2 + eyeRx * 0.7} ${eyeY + eyeRy * 2} 
                        ${cx + sep / 2 + eyeRx * 0.8} ${eyeY + eyeRy * 2.5}`}
                  style={{ stroke: '#4A90E2', strokeWidth: 2, fill: 'none', opacity: 0.3, strokeLinecap: 'round' }} />
        ...
        {isMad && sunglassesType === 0 && (
          <>
            <path
              ...
              style={{ stroke: '#000000', strokeWidth: 3, strokeLinecap: 'round' }}
            />
            <path
              d={`M ${cx + sep / 2 + eyeRx * 1.2} ${eyeY - eyeRy * 1.5}
                  L ${cx + sep / 2 - eyeRx * 0.8} ${eyeY - eyeRy * 0.8}`}
        ...
        <Lips
          x1={cx - lipW / 2}
          y={isFullFace ? SIZE * (0.62 + (getUnit(n + 81, 11) - 5) / 100) : cy + SIZE * 0.12}
          x2={cx + lipW / 2}
          curve={lipCurve}
