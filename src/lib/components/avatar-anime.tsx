...
export default function AvatarAnime({
  ...
  const mouthW = SIZE * 0.2; // Wider mouth
  ...
  const MouthO = ({ filled }: { filled?: boolean }) => (
    <ellipse
      ...
      ry={mouthH * 0.9}
      style={{
        fill: filled ? String(features) : 'none',
        stroke: String(features),
        strokeWidth: 2,
      }}
    />
  ...
  return (
    ...
    >
      ...
      <g mask={`url(#${maskID})`}>
        ...
        <radialGradient cx="50%" cy="40%" id={`g-${maskID}`}>
          <stop offset="0%" stopColor="white" stopOpacity="0.08" />
          <stop offset="100%" stopColor="black" stopOpacity="0" />
        </radialGradient>
        <rect fill={`url(#g-${maskID})`} height={SIZE} width={SIZE} />
          ...
          {showBlush && !isKodama && (
            <>
              <circle
                ...
                cy={cy}
                r={SIZE * 0.05}
                style={{ fill: String(accent), opacity: 0.5 }}
              />
              <circle
                cx={cx + SIZE * 0.22}
                cy={cy}
        ...
        {/* Close features transform group */}
      </g>{' '}
      {/* Close mask group */}
    </svg>
  );
